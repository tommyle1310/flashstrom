    ...
    async notifyPartiesOnce(order) {
        ...
        try {
            ...
            if (!driver && order.driver_id) {
                console.warn(`[DEBUG] notifyPartiesOnce - order.driver is null, fetching driver with id: ${order.driver_id}`);
        ...
        try {
            const result = await this.dataSource.transaction(async (transactionalEntityManager) => {
                ...
                if (allDeliveryCompleteDone &&
                    ...
                    console.log('check total earns', dps.total_earns, 'check driver wallet', driverWallet.id);
                ...
                if (!updatedOrder.driver && updatedOrder.driver_id) {
                    console.warn(`[DEBUG] handleDriverProgressUpdate - updatedOrder.driver is null, fetching driver with id: ${updatedOrder.driver_id}`);
                    updatedOrder.driver = await transactionalEntityManager
                        .getRepository(driver_entity_1.Driver)
                        .findOne({ where: { id: updatedOrder.driver_id } });
                    if (!updatedOrder.driver) {
                        console.error(`[DEBUG] handleDriverProgressUpdate - Driver ${updatedOrder.driver_id} not found`);
                    }
                }
                console.log('[DEBUG] handleDriverProgressUpdate - updatedOrder.driver:', updatedOrder.driver);
